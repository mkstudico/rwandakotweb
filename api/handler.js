...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (path === '/api/download' && method === 'POST') {
            ...
            if (!signed) return res.status(500).json({ error: 'Link generation failed' });
        ...
        if (path === '/api/user-payments' && method === 'POST') {
            ...
            const { data, error } = await supabase.from('payments')
                .select(`
                    id, document_id, amount, currency, status,
                    transaction_ref, payment_method, downloaded,
        ...
        if (path === '/api/callback' && method === 'POST') {
            ...
            const { data: p } = await supabase.from('payments').select('id,amount,status').eq('client_token', client_token).single();
            ...
            if (status === 'success') {
                await supabase.from('payments').update({
                    ...
                    payment_method: payment_method || null, amount: parseInt(amount) || p.amount
