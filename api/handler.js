...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (path === '/api/rwandapay-init' && method === 'POST') {
            ...
            if (!vu(document_id) || !vf(device_fingerprint) || !vphone(phone) || !vt(client_token)) {
            ...
            await supabase.from('payments').insert({
                ...
                amount: doc.price, currency: doc.currency,
            ...
            const cemail = cname.toLowerCase().replace(/[^a-z0-9]/g, '') + '@kotweb.rw';

            const rpBody = {
                amount: parseInt(doc.price),
                tx_ref: client_token,
                currency: doc.currency || 'RWF',
                customer: { name: cname, email: cemail, phone: np },
                ...
                description: doc.title || 'Document Purchase',
                meta: { document_id, device_fingerprint }
            };
            ...
            try {
                const rpRes = await fetch(`${RWANDAPAY_BASE}/checkout/initialize`, {
                    method: 'POST',
                    headers: {
                        'X-Public-Key': RWANDAPAY_PUBLIC_KEY,
                        'X-Secret-Key': RWANDAPAY_SECRET_KEY,
                ...
                if (rpData.success && rpData.data?.payment_url) {
                    await supabase.from('payments').update({
                        transaction_ref: rpData.data.reference || client_token
                    }).eq('client_token', client_token);

                    return res.status(200).json({
                        success: true,
                        payment_url: rpData.data.payment_url,
                        reference: rpData.data.reference || client_token
                    });
                }

                return res.status(500).json({ error: rpData.message || 'Payment initialization failed' });
            } catch (err) {
        ...
        // ═══════════════════════════════════════════
        // RWANDAPAY: Verify payment status
        // ═══════════════════════════════════════════
        if (path === '/api/rwandapay-verify' && method === 'POST') {
            ...
            try {
                const rpRes = await fetch(`${RWANDAPAY_BASE}/checkout/${ref}/verify`, {
                ...
                if (rpData.completed && rpData.success && rpData.status === 'successful') {
                    await supabase.from('payments').update({
                        status: 'completed', payment_method: 'rwandapay'
                    }).eq('client_token', ref).eq('status', 'pending');

                    await supabase.from('payments').update({
                        status: 'completed', payment_method: 'rwandapay'
                    }).eq('transaction_ref', ref).eq('status', 'pending');
        ...
        if (path === '/api/rwandapay-callback' && method === 'POST') {
            ...
            if ((event === 'payment.successful' || event === 'payment.success') && data?.reference) {
                ...
                const { data: payment } = await supabase
                    .from('payments').select('id, status').eq('client_token', ref).single();

                if (!payment) {
                    const { data: p2 } = await supabase
                        .from('payments').select('id, status').eq('transaction_ref', ref).single();
                    if (p2 && p2.status === 'pending') {
                        await supabase.from('payments').update({
                            status: 'completed',
                            transaction_ref: data.transaction_id || ref,
                            payment_method: 'rwandapay'
                        }).eq('id', p2.id);
                    }
            ...
            if ((event === 'payment.failed' || event === 'payment.fail') && data?.reference) {
                await supabase.from('payments').update({ status: 'failed' })
                    .eq('client_token', data.reference).eq('status', 'pending');
            }
        ...
        if (path === '/api/check-payment' && method === 'POST') {
            ...
            if (device_fingerprint && document_id) {
                ...
                if (payment?.created_at && payment.status === 'completed') {
                    ww = (Date.now() - new Date(payment.created_at).getTime()) / 60000 <= DOWNLOAD_WINDOW_MINUTES;
                }

                return res.status(200).json({
                    already_paid: found && payment.status === 'completed' && ww,
                    payment,
                    within_window: ww
                });
            }
        ...
        if (path === '/api/download' && method === 'POST') {
            ...
            if (!vf(device_fingerprint) || !vu(document_id)) {
                return res.status(400).json({ error: 'Invalid device or document parameters' });
            }
            ...
            if (payErr || !payment) {
                return res.status(403).json({ error: 'No completed payment found. Please complete payment first.' });
            }

            const diffMin = (Date.now() - new Date(payment.created_at).getTime()) / 60000;
            if (diffMin > DOWNLOAD_WINDOW_MINUTES) {
                return res.status(410).json({ error: 'Download window expired. Please purchase again.', expired: true });
            }
            ...
            if (signedErr || !signedData) return res.status(500).json({ error: 'Failed to generate download link' });
            ...
            return res.status(200).json({
                ...
                expires_in_minutes: Math.max(0, Math.floor(DOWNLOAD_WINDOW_MINUTES - diffMin))
            });
        }
        ...
        if (path === '/api/user-payments' && method === 'POST') {
            ...
            for (const p of (data || [])) {
                ...
                enriched.push({
                    ...p,
                    document_title: d?.title || 'Unknown Document',
                    document_image: d?.image_url || ''
                });
            }
        ...
        // ═══════════════════════════════════════════
        // PUBLIC: Verify payment with proof (OCR)
        // ═══════════════════════════════════════════
        if (path === '/api/verify-with-proof' && method === 'POST') {
            ...
            const { data: payment } = await supabase
                .from('payments')
                .select('id, document_id, amount, currency, status, documents!inner(title)')
                .eq('client_token', client_token).single();
            ...
            await supabase.from('payment_verifications').insert({
                payment_id: payment.id, client_token,
            ...
            if (is_suspicious) {
                const np = payment.amount * 2;
                await supabase.from('payments').update({
                    amount: np, status: 'pending_verification'
                }).eq('client_token', client_token);

                return res.status(200).json({
                    verified: false, suspicious: true,
                    original_price: payment.amount, new_price: np,
        ...
        if (path === '/api/admin/upload' && method === 'POST') {
            ...
            const fb = Buffer.concat(chunks);
            const eb = Buffer.from('--' + boundary + '--');
            ...
            let cs = -1;
            for (let i = 0; i < fb.length - 4; i++) {
                if (fb[i]===0x0d&&fb[i+1]===0x0a&&fb[i+2]===0x0d&&fb[i+3]===0x0a) { cs = i + 4; break; }
            }
            if (cs === -1 || !fn) return res.status(400).json({ error: 'Parse failed' });

            let ce = fb.length;
            for (let i = fb.length - eb.length; i >= 0; i--) {
                if (fb.slice(i, i + eb.length).equals(eb)) { ce = i - 2; break; }
            }

            const bf = fb.slice(cs, ce);
            if (!bf || bf.length === 0) return res.status(400).json({ error: 'Empty file' });

            const sn = Date.now() + '_' + fn.replace(/[^a-zA-Z0-9._-]/g, '_');
            const { error } = await supabase.storage.from('documents').upload(sn, bf, {
                contentType: ft, cacheControl: '3600', upsert: false
            });
        ...
        if (path === '/api/admin/documents' && method === 'POST') {
            ...
            if (!title || !file_path || !price) return res.status(400).json({ error: 'Missing fields' });
        ...
        if (path.match(/^\/api\/admin\/documents\/[a-f0-9-]+\/toggle$/) && method === 'PUT') {
            ...
            if (error) return res.status(500).json({ error: 'Toggle failed' });
        ...
        if (path === '/api/admin/verifications' && method === 'GET') {
            if (!(await isAdmin(req))) return res.status(401).json({ error: 'Unauthorized' });
            const { data, error } = await supabase
                .from('payment_verifications').select('*').order('verified_at', { ascending: false }).limit(100);
            if (error) return res.status(500).json({ error: 'Fetch failed' });
            return res.status(200).json(data || []);
        }
