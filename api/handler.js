...
export default async function handler(req, res) {
    ...
    if (method === 'POST' && path !== '/api/rwandapay-callback' && !path.startsWith('/api/admin/upload')) {
        if (!rateLimit(ip)) return res.status(429).json({ error: 'Too many requests' });
    }
    try {
        // GET all documents
        if (path === '/api/documents' && method === 'GET') {
            const { data, error } = await supabase.from('documents').select('id,title,description,image_url,price,currency,clicks,created_at').eq('active', true).order('created_at', { ascending: false });
        ...
        if (path.match(/^\/api\/documents\/[a-f0-9-]+$/) && method === 'GET') {
            ...
            const { data, error } = await supabase.from('documents').select('id,title,description,image_url,price,currency,clicks,created_at').eq('id', docId).eq('active', true).single();
        ...
        if (path === '/api/rwandapay-init' && method === 'POST') {
            ...
            if (!vu(document_id) || !vf(device_fingerprint) || !vphone(phone) || !vt(client_token)) return res.status(400).json({ error: 'Invalid parameters' });
        ...
        if (path === '/api/rwandapay-verify' && method === 'POST') {
            ...
            try {
                ...
                if (rpData.completed && !rpData.success) {
                    ...
                    await supabase.from('payments').update({ status: 'failed' }).eq('transaction_ref', ref).eq('status', 'pending');
        ...
        if (path === '/api/rwandapay-callback' && method === 'POST') {
            let payload;
            try { const chunks = []; for await (const c of req) chunks.push(c); payload = JSON.parse(Buffer.concat(chunks).toString()); } catch (e) { return res.status(200).json({ received: true }); }
            ...
            if ((event === 'payment.successful' || event === 'payment.success') && data?.reference) {
                ...
                if (!payment) {
                    ...
                    await supabase.from('payments').update({ status: 'completed', transaction_ref: data.transaction_id || ref, payment_method: 'rwandapay' }).eq('id', payment.id);
                }
            }
            if ((event === 'payment.failed' || event === 'payment.fail') && data?.reference) {
                await supabase.from('payments').update({ status: 'failed' }).eq('client_token', data.reference).eq('status', 'pending');
            }
        ...
        if (path === '/api/user-payments' && method === 'POST') {
            ...
            const enriched = [];
            for (const p of (data || [])) { const { data: d } = await supabase.from('documents').select('title, image_url').eq('id', p.document_id).single(); enriched.push({ ...p, document_title: d?.title || 'Unknown', document_image: d?.image_url || '' }); }
