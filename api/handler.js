...
export default async function handler(req, res) {
    ...
    if (method === 'POST' && path !== '/api/rwandapay-callback' && !path.startsWith('/api/admin/upload') && !path.startsWith('/api/pub/upload')) { if (!rateLimit(ip)) return res.status(429).json({ error: 'Too many requests' }); }
    try {
        ...
        if (path === '/api/rwandapay-init' && method === 'POST') {
            ...
            await supabase.from('payments').insert({ document_id, device_fingerprint, client_token, amount: finalAmount, currency: doc.currency, status: 'pending', payment_method: 'rwandapay' });
            ...
            try {
                ...
                if (!rpData.success || !rpData.data?.payment_url) { console.error('RP error:', rpData); return res.status(500).json({ error: rpData.message || 'Payment failed' }); }
            ...
            } catch (err) { console.error('RP fetch:', err); return res.status(500).json({ error: 'Service unavailable' }); }
        }
        ...
        if (path === '/api/rwandapay-verify' && method === 'POST') {
            ...
            try {
                ...
                if (rpData.completed && !rpData.success) { await supabase.from('payments').update({ status: 'failed' }).eq('client_token', ref).eq('status', 'pending'); return res.status(200).json({ verified: false, status: 'failed' }); }
        ...
        if (path === '/api/rwandapay-callback' && method === 'POST') {
            ...
            if ((event === 'payment.successful' || event === 'payment.success') && data?.reference) { await supabase.from('payments').update({ status: 'completed', payment_method: 'rwandapay' }).or(`client_token.eq.${data.reference},transaction_ref.eq.${data.reference}`).eq('status', 'pending'); await supabase.from('pub_payments').update({ status: 'completed', payment_method: 'rwandapay' }).or(`client_token.eq.${data.reference},transaction_ref.eq.${data.reference}`).eq('status', 'pending'); }
            if ((event === 'payment.failed' || event === 'payment.fail') && data?.reference) { await supabase.from('payments').update({ status: 'failed' }).or(`client_token.eq.${data.reference},transaction_ref.eq.${data.reference}`).eq('status', 'pending'); await supabase.from('pub_payments').update({ status: 'failed' }).or(`client_token.eq.${data.reference},transaction_ref.eq.${data.reference}`).eq('status', 'pending'); }
        ...
        if (path === '/api/pub/earnings' && method === 'GET') {
            ...
            const total = realPayments.reduce((s,p) => s + p.amount, 0);
            const fees = realPayments.reduce((s,p) => s + p.platform_fee, 0);
            const { count: docCount } = await supabase.from('pub_documents').select('id', { count: 'exact' }).eq('publisher_id', user.id);
            return res.status(200).json({ total_sales: total, your_earnings: pub?.balance || 0, platform_fees: fees, total_documents: docCount||0, total_transactions: realPayments.length, recent: (payments||[]).slice(0, 10), bonus_claimed: pub?.bonus_claimed || false });
        }
        ...
        if (path === '/api/pub/init-payment' && method === 'POST') {
            const { document_id, device_fingerprint, phone, client_token, customer_name } = req.body;
            if (!vu(document_id) || !vf(device_fingerprint) || !vphone(phone) || !vt(client_token)) return res.status(400).json({ error: 'Invalid params' });
            const { data: doc } = await supabase.from('pub_documents').select('*, publishers!inner(id, balance)').eq('id', document_id).eq('active', true).single();
            if (!doc) return res.status(404).json({ error: 'Not found' });
            const finalAmount = Math.max(100, parseInt(doc.price));
            ...
            let np = phone.replace(/\s/g, '').replace('+250', ''); if (np.startsWith('0')) np = np.substring(1);
            const cname = (customer_name || '').trim() || 'Customer';
            const rpBody2 = { amount: finalAmount, tx_ref: client_token, currency: doc.currency || 'RWF', customer: { name: cname, email: cname.toLowerCase().replace(/[^a-z0-9]/g,'')+'@kotweb.rw', phone: np }, redirect_url: `${req.headers['x-forwarded-proto']||'https'}://${req.headers.host}/pub-view.html?doc=${document_id}&rp_ref=${client_token}`, webhook_url: `${req.headers['x-forwarded-proto']||'https'}://${req.headers.host}/api/rwandapay-callback`, description: doc.title || 'Purchase' };
            try { const rpRes2 = await fetch(`${RWANDAPAY_BASE}/checkout/initialize`, { method: 'POST', headers: { 'X-Public-Key': RWANDAPAY_PUBLIC_KEY, 'X-Secret-Key': RWANDAPAY_SECRET_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify(rpBody2) }); const rpData2 = await rpRes2.json(); if (!rpData2.success || !rpData2.data?.payment_url) return res.status(500).json({ error: rpData2.message || 'Payment failed' }); await supabase.from('pub_payments').update({ transaction_ref: rpData2.data.reference || client_token }).eq('client_token', client_token); return res.status(200).json({ success: true, payment_url: rpData2.data.payment_url, reference: rpData2.data.reference || client_token }); } catch (err) { return res.status(500).json({ error: 'Service unavailable' }); }
        }

        if (path === '/api/pub/verify-payment' && method === 'POST') {
            const { reference, client_token, document_id, device_fingerprint } = req.body; const ref = reference || client_token;
            try { const rpRes2 = await fetch(`${RWANDAPAY_BASE}/checkout/${ref}/verify`, { headers: { 'Accept': 'application/json' } }); const rpData2 = await rpRes2.json();
                if (rpData2.completed && rpData2.success && rpData2.status === 'successful') {
