...
export default async function handler(req, res) {
    ...
    if (method === 'POST' && !path.startsWith('/api/pub/upload')) { if (!rateLimit(ip)) return res.status(429).json({ error: 'Too many requests' }); }

    try {

        // PUB: Get single document
        if (path.match(/^\/api\/pub\/document\/[a-f0-9-]+$/) && method === 'GET') {
            const docId = path.split('/').pop(); if (!vu(docId)) return res.status(400).json({ error: 'Invalid ID' });
        ...
        // PUB: Upload file
        if (path === '/api/pub/upload' && method === 'POST') { const user = pubAuth(req); if (!user) return res.status(401).json({ error: 'Unauthorized' }); const ct = req.headers['content-type'] || ''; if (!ct.includes('multipart/form-data')) return res.status(400).json({ error: 'Expected file' }); const boundary = ct.split('boundary=')[1]; if (!boundary) return res.status(400).json({ error: 'No boundary' }); const chunks = []; for await (const c of req) chunks.push(c); const fb = Buffer.concat(chunks), eb = Buffer.from('--' + boundary + '--'); let fn = null, ft = 'application/octet-stream'; const hs = fb.toString('utf8', 0, Math.min(fb.length, 4096)); const fm = hs.match(/filename="(.+?)"/); if (fm) fn = fm[1]; const cm = hs.match(/Content-Type: (.+?)\r\n/); if (cm) ft = cm[1].trim(); let cs = -1; for (let i = 0; i < fb.length - 4; i++) { if (fb[i]===0x0d&&fb[i+1]===0x0a&&fb[i+2]===0x0d&&fb[i+3]===0x0a) { cs = i + 4; break; } } if (cs === -1 || !fn) return res.status(400).json({ error: 'Parse failed' }); let ce = fb.length; for (let i = fb.length - eb.length; i >= 0; i--) { if (fb.slice(i, i + eb.length).equals(eb)) { ce = i - 2; break; } } const bf = fb.slice(cs, ce); if (!bf || bf.length === 0) return res.status(400).json({ error: 'Empty file' }); const sn = `pub_${user.id}_${Date.now()}_${fn.replace(/[^a-zA-Z0-9._-]/g, '_')}`; const { error } = await supabase.storage.from(PUB_BUCKET).upload(sn, bf, { contentType: ft, cacheControl: '3600', upsert: false }); if (error) return res.status(500).json({ error: 'Upload failed' }); return res.status(200).json({ file_path: sn, filename: fn, size: bf.length }); }

        // PUB: Earnings
        if (path === '/api/pub/earnings' && method === 'GET') { const user = pubAuth(req); if (!user) return res.status(401).json({ error: 'Unauthorized' }); const { data: payments } = await supabase.from('pub_payments').select('amount, publisher_earnings, platform_fee, customer_name, customer_phone, created_at').eq('publisher_id', user.id).eq('status', 'completed').order('created_at', { ascending: false }); const total = (payments||[]).reduce((s,p) => s + p.amount, 0); const earnings = (payments||[]).reduce((s,p) => s + p.publisher_earnings, 0); const fees = (payments||[]).reduce((s,p) => s + p.platform_fee, 0); const { count: docCount } = await supabase.from('pub_documents').select('id', { count: 'exact' }).eq('publisher_id', user.id); return res.status(200).json({ total_sales: total, your_earnings: earnings, platform_fees: fees, total_documents: docCount||0, total_transactions: (payments||[]).length, recent: (payments||[]).slice(0, 10) }); }

        // PUB: Public page
        if (path.match(/^\/api\/pub\/page\/[a-z0-9-]+$/) && method === 'GET') { const slug = path.split('/').pop(); const { data: pub } = await supabase.from('publishers').select('id, username, name, slug').eq('slug', slug).single(); if (!pub) return res.status(404).json({ error: 'Publisher not found' }); const { data: docs } = await supabase.from('pub_documents').select('id, title, description, image_url, price, currency, clicks, created_at').eq('publisher_id', pub.id).eq('active', true).order('created_at', { ascending: false }); return res.status(200).json({ publisher: pub, documents: docs || [] }); }

        // PUB: Init payment
        if (path === '/api/pub/init-payment' && method === 'POST') { const { document_id, device_fingerprint, phone, client_token, customer_name } = req.body; if (!vu(document_id) || !vf(device_fingerprint) || !vphone(phone) || !vt(client_token)) return res.status(400).json({ error: 'Invalid params' }); const { data: doc } = await supabase.from('pub_documents').select('*, publishers!inner(id)').eq('id', document_id).eq('active', true).single(); if (!doc) return res.status(404).json({ error: 'Not found' }); const finalAmount = Math.max(100, parseInt(doc.price)); const pubEarnings = Math.floor(finalAmount * 0.93); const platFee = finalAmount - pubEarnings; await supabase.from('pub_payments').insert({ document_id, publisher_id: doc.publisher_id, device_fingerprint, client_token, amount: finalAmount, publisher_earnings: pubEarnings, platform_fee: platFee, currency: doc.currency, status: 'pending', customer_name: (customer_name||'').trim(), customer_phone: phone }); let np = phone.replace(/\s/g, '').replace('+250', ''); if (np.startsWith('0')) np = np.substring(1); const cname = (customer_name || '').trim() || 'Customer'; const rpBody = { amount: finalAmount, tx_ref: client_token, currency: doc.currency || 'RWF', customer: { name: cname, email: cname.toLowerCase().replace(/[^a-z0-9]/g,'')+'@kotweb.rw', phone: np }, redirect_url: `${req.headers['x-forwarded-proto']||'https'}://${req.headers.host}/pub-view.html?doc=${document_id}&rp_ref=${client_token}`, webhook_url: `${req.headers['x-forwarded-proto']||'https'}://${req.headers.host}/api/rwandapay-callback`, description: doc.title || 'Purchase' }; try { const rpRes = await fetch(`${RWANDAPAY_BASE}/checkout/initialize`, { method: 'POST', headers: { 'X-Public-Key': RWANDAPAY_PUBLIC_KEY, 'X-Secret-Key': RWANDAPAY_SECRET_KEY, 'Content-Type': 'application/json' }, body: JSON.stringify(rpBody) }); const rpData = await rpRes.json(); if (!rpData.success || !rpData.data?.payment_url) return res.status(500).json({ error: rpData.message || 'Payment failed' }); await supabase.from('pub_payments').update({ transaction_ref: rpData.data.reference || client_token }).eq('client_token', client_token); return res.status(200).json({ success: true, payment_url: rpData.data.payment_url, reference: rpData.data.reference || client_token }); } catch (err) { return res.status(500).json({ error: 'Service unavailable' }); } }

        // PUB: Verify payment
        if (path === '/api/pub/verify-payment' && method === 'POST') { const { reference, client_token, document_id, device_fingerprint } = req.body; const ref = reference || client_token; try { const rpRes = await fetch(`${RWANDAPAY_BASE}/checkout/${ref}/verify`, { headers: { 'Accept': 'application/json' } }); const rpData = await rpRes.json(); if (rpData.completed && rpData.success && rpData.status === 'successful') { await supabase.from('pub_payments').update({ status: 'completed', payment_method: 'rwandapay' }).eq('client_token', ref).eq('status', 'pending'); await supabase.from('pub_payments').update({ status: 'completed' }).eq('transaction_ref', ref).eq('status', 'pending'); if (device_fingerprint && document_id) await supabase.from('pub_payments').update({ status: 'completed' }).eq('device_fingerprint', device_fingerprint).eq('document_id', document_id).eq('status', 'pending'); return res.status(200).json({ verified: true, status: 'completed', document_id }); } if (rpData.completed && !rpData.success) { await supabase.from('pub_payments').update({ status: 'failed' }).eq('client_token', ref).eq('status', 'pending'); return res.status(200).json({ verified: false, status: 'failed' }); } return res.status(200).json({ verified: false, status: rpData.status || 'pending' }); } catch (err) { return res.status(500).json({ error: 'Verify failed' }); } }

        // PUB: Download
        if (path === '/api/pub/download' && method === 'POST') { const { device_fingerprint, document_id } = req.body; if (!vf(device_fingerprint) || !vu(document_id)) return res.status(400).json({ error: 'Invalid parameters' }); const { data: payment } = await supabase.from('pub_payments').select('*').eq('device_fingerprint', device_fingerprint).eq('document_id', document_id).eq('status', 'completed').order('created_at', { ascending: false }).limit(1).single(); if (!payment) return res.status(403).json({ error: 'No completed payment found.' }); const diffMin = (Date.now() - new Date(payment.created_at).getTime()) / 60000; if (diffMin > DOWNLOAD_WINDOW_MINUTES) return res.status(410).json({ error: 'Expired', expired: true }); const { data: doc } = await supabase.from('pub_documents').select('file_path').eq('id', document_id).single(); if (!doc) return res.status(404).json({ error: 'Not found' }); const { data: signedData } = await supabase.storage.from(PUB_BUCKET).createSignedUrl(doc.file_path, 600); if (!signedData) return res.status(500).json({ error: 'Link failed' }); await supabase.from('pub_payments').update({ downloaded: true }).eq('id', payment.id); return res.status(200).json({ url: signedData.signedUrl, filename: doc.file_path.split('/').pop() }); }
    ...
    } catch (err) { console.error('[Pub Handler]', err); return res.status(500).json({ error: 'Internal error' }); }
}
