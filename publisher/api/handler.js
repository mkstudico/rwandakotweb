...
if (path === '/api/pub/register' && method === 'POST') {
    ...
    if (!username || !email || !password || !name) return res.status(400).json({ error: 'All fields required' });
    ...
    const { data, error } = await supabase.from('publishers').insert({ username, email, password_hash: hash, name, slug }).select().single();
...
// PUB: Get publisher page data (public)
