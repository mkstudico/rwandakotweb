...
const PUB_BUCKET = process.env.PUB_STORAGE_BUCKET || 'pub-documents';
...
function pubAuth(req) { const auth = req.headers.authorization; if (!auth?.startsWith('Bearer ')) return null; return pubVerifyToken(auth.slice(7)); }
...
export default async function handler(req, res) {
    ...
    if (method === 'POST' && path !== '/api/rwandapay-callback' && !path.startsWith('/api/admin/upload') && !path.startsWith('/api/pub/upload')) {
    ...
    try {
        ...
        // ═══════════════════════════════════════════
        // MAIN STORE: GET single document
        // ═══════════════════════════════════════════
        ...
        if (path === '/api/rwandapay-init' && method === 'POST') {
            ...
            if (!RWANDAPAY_PUBLIC_KEY || !RWANDAPAY_SECRET_KEY) return res.status(500).json({ error: 'Payment config error' });

            const { data: exist } = await supabase.from('payments').select('id, created_at')
                .eq('device_fingerprint', device_fingerprint).eq('document_id', document_id).eq('status', 'completed').limit(1);
            if (exist?.length && (Date.now() - new Date(exist[0].created_at).getTime()) / 60000 <= DOWNLOAD_WINDOW_MINUTES)
            ...
            await supabase.from('payments').insert({
                ...
                amount: finalAmount, currency: doc.currency,
                status: 'pending', payment_method: 'rwandapay'
            });

            let np = phone.replace(/\s/g, '').replace('+250', '');
            if (np.startsWith('0')) np = np.substring(1);
            const cname = (customer_name || '').trim() || 'Customer';

            const rpBody = {
                amount: finalAmount, tx_ref: client_token, currency: doc.currency || 'RWF',
                customer: { name: cname, email: cname.toLowerCase().replace(/[^a-z0-9]/g, '') + '@kotweb.rw', phone: np },
                redirect_url: `${req.headers['x-forwarded-proto'] || 'https'}://${req.headers.host}/view.html?doc=${document_id}&rp_ref=${client_token}`,
                webhook_url: `${req.headers['x-forwarded-proto'] || 'https'}://${req.headers.host}/api/rwandapay-callback`,
                description: doc.title || 'Purchase', meta: { document_id, device_fingerprint }
            };

            try {
                const rpRes = await fetch(`${RWANDAPAY_BASE}/checkout/initialize`, {
                    method: 'POST',
                    headers: { 'X-Public-Key': RWANDAPAY_PUBLIC_KEY, 'X-Secret-Key': RWANDAPAY_SECRET_KEY, 'Content-Type': 'application/json' },
                    body: JSON.stringify(rpBody)
                });
                const rpData = await rpRes.json();
                if (!rpData.success || !rpData.data?.payment_url) {
                    return res.status(500).json({ error: rpData.message || 'Payment failed' });
                }
        ...
        if (path === '/api/rwandapay-verify' && method === 'POST') {
            ...
            try {
                ...
                if (rpData.completed && rpData.success && rpData.status === 'successful') {
                    ...
                    if (device_fingerprint && document_id) await supabase.from('payments').update({ status: 'completed', payment_method: 'rwandapay' }).eq('device_fingerprint', device_fingerprint).eq('document_id', document_id).eq('status', 'pending');
                    return res.status(200).json({ verified: true, status: 'completed', document_id });
                }
                if (rpData.completed && !rpData.success) {
                    await supabase.from('payments').update({ status: 'failed' }).eq('client_token', ref).eq('status', 'pending');
        ...
        if (path === '/api/rwandapay-callback' && method === 'POST') {
            ...
            } catch (e) { return res.status(200).json({ received: true }); }

            const event = payload.event, data = payload.data;
            if ((event === 'payment.successful' || event === 'payment.success') && data?.reference) {
                await supabase.from('payments').update({ status: 'completed', payment_method: 'rwandapay' }).or(`client_token.eq.${data.reference},transaction_ref.eq.${data.reference}`).eq('status', 'pending');
                await supabase.from('pub_payments').update({ status: 'completed', payment_method: 'rwandapay' }).or(`client_token.eq.${data.reference},transaction_ref.eq.${data.reference}`).eq('status', 'pending');
            }
            if ((event === 'payment.failed' || event === 'payment.fail') && data?.reference) {
                await supabase.from('payments').update({ status: 'failed' }).or(`client_token.eq.${data.reference},transaction_ref.eq.${data.reference}`).eq('status', 'pending');
                await supabase.from('pub_payments').update({ status: 'failed' }).or(`client_token.eq.${data.reference},transaction_ref.eq.${data.reference}`).eq('status', 'pending');
            }
            return res.status(200).json({ received: true });
        }

        // ═══════════════════════════════════════════
        // MAIN STORE: Check payment status
        // ═══════════════════════════════════════════
        if (path === '/api/check-payment' && method === 'POST') {
            ...
            if (client_token) {
                ...
                const { data } = await supabase.from('payments').select('status, document_id, amount, currency, created_at').eq('client_token', client_token).single();
                if (!data) return res.status(404).json({ error: 'Not found' });
                return res.status(200).json({ paid: data.status === 'completed', status: data.status, document_id: data.document_id, amount: data.amount, currency: data.currency, created_at: data.created_at });
            }
            if (device_fingerprint && document_id) {
                if (!vf(device_fingerprint) || !vu(document_id)) return res.status(400).json({ error: 'Invalid params' });
                const { data } = await supabase.from('payments').select('id, status, amount, created_at').eq('device_fingerprint', device_fingerprint).eq('document_id', document_id).order('created_at', { ascending: false }).limit(1);
                const found = data?.length > 0, payment = found ? data[0] : null;
                let ww = false;
                if (payment?.created_at && payment.status === 'completed') ww = (Date.now() - new Date(payment.created_at).getTime()) / 60000 <= DOWNLOAD_WINDOW_MINUTES;
                return res.status(200).json({ already_paid: found && payment.status === 'completed' && ww, payment, within_window: ww });
            }
            return res.status(400).json({ error: 'Missing params' });
        }

        // ═══════════════════════════════════════════
        // MAIN STORE: Download document
        // ═══════════════════════════════════════════
        if (path === '/api/download' && method === 'POST') {
            ...
            const { data: payment } = await supabase.from('payments').select('*')
                .eq('device_fingerprint', device_fingerprint).eq('document_id', document_id)
        ...
        // ═══════════════════════════════════════════
        // MAIN STORE: User payments history
        // ═══════════════════════════════════════════
        if (path === '/api/user-payments' && method === 'POST') {
            ...
            const { data, error } = await supabase.from('payments').select('id, document_id, client_token, amount, currency, status, transaction_ref, payment_method, downloaded, created_at, downloaded_at').eq('device_fingerprint', device_fingerprint).order('created_at', { ascending: false }).limit(50);
            if (error) return res.status(500).json({ error: 'Fetch failed' });
            const enriched = [];
            for (const p of (data || [])) {
                const { data: d } = await supabase.from('documents').select('title, image_url').eq('id', p.document_id).single();
                enriched.push({ ...p, document_title: d?.title || 'Unknown', document_image: d?.image_url || '' });
            }
            return res.status(200).json(enriched);
        }

        // ═══════════════════════════════════════════
        // MAIN STORE: Verify with proof (OCR)
        // ═══════════════════════════════════════════
        if (path === '/api/verify-with-proof' && method === 'POST') {
            ...
            const { data: payment } = await supabase.from('payments').select('id, document_id, amount, currency, status, documents!inner(title)').eq('client_token', client_token).single();
            if (!payment) return res.status(404).json({ error: 'Not found' });
            if (payment.status === 'completed') return res.status(200).json({ verified: true, suspicious: false, document_id: payment.document_id });
            await supabase.from('payment_verifications').insert({ payment_id: payment.id, client_token, device_fingerprint: device_fingerprint || '', buyer_name: buyer_name || '', phone_number: phone_number || '', ocr_text: ocr_text || '', match_percentage: match_percentage || 0, is_suspicious: is_suspicious || false, verified_at: new Date().toISOString() });
            if (is_suspicious) { const np = payment.amount * 2; await supabase.from('payments').update({ amount: np, status: 'pending_verification' }).eq('client_token', client_token); return res.status(200).json({ verified: false, suspicious: true, original_price: payment.amount, new_price: np, currency: payment.currency, document_id: payment.document_id }); }
            await supabase.from('payments').update({ status: 'completed', payment_method: 'verified_manual' }).eq('client_token', client_token).eq('status', 'pending');
            return res.status(200).json({ verified: true, suspicious: false, document_id: payment.document_id });
        }

        // ═══════════════════════════════════════════
        // PUBLISHER: Get single document
        // ═══════════════════════════════════════════
        ...
        if (path === '/api/pub/register' && method === 'POST') {
            ...
            const { data, error } = await supabase.from('publishers').insert({ username, email, password_hash: hash, name: name || username, slug }).select().single();
        ...
        // ═══════════════════════════════════════════
        // PUBLISHER: Add document
        // ═══════════════════════════════════════════
        if (path === '/api/pub/documents' && method === 'POST') {
            const user = pubAuth(req); if (!user) return res.status(401).json({ error: 'Unauthorized' });
            const { title, description, image_url, file_path, price } = req.body;
            if (!title || !file_path || !price || parseInt(price) < 100) return res.status(400).json({ error: 'Title, file, and price (min 100) required' });
            const { data, error } = await supabase.from('pub_documents').insert({ publisher_id: user.id, title: title.trim(), description: (description||'').trim(), image_url: (image_url||'').trim(), file_path, price: parseInt(price), active: true }).select().single();
            if (error) return res.status(500).json({ error: 'Insert failed' });
            return res.status(201).json(data);
        }

        // ═══════════════════════════════════════════
        // PUBLISHER: Delete document
        // ═══════════════════════════════════════════
        if (path.match(/^\/api\/pub\/documents\/[a-f0-9-]+$/) && method === 'DELETE') {
            ...
            if (!doc) return res.status(404).json({ error: 'Not found' });
            if (doc.file_path) await supabase.storage.from(PUB_BUCKET).remove([doc.file_path]);
            await supabase.from('pub_documents').delete().eq('id', docId);
            return res.status(200).json({ success: true });
        }

        // ═══════════════════════════════════════════
        // PUBLISHER: Toggle document
        // ═══════════════════════════════════════════
        ...
        if (path === '/api/pub/upload' && method === 'POST') {
            ...
            const { error } = await supabase.storage.from(PUB_BUCKET).upload(sn, bf, { contentType: ft, cacheControl: '3600', upsert: false });
        ...
        if (path === '/api/pub/earnings' && method === 'GET') {
            const user = pubAuth(req); if (!user) return res.status(401).json({ error: 'Unauthorized' });
            const { data: payments } = await supabase.from('pub_payments').select('amount, publisher_earnings, platform_fee, customer_name, customer_phone, created_at').eq('publisher_id', user.id).eq('status', 'completed').order('created_at', { ascending: false });
            const total = (payments||[]).reduce((s,p) => s + p.amount, 0);
            const earnings = (payments||[]).reduce((s,p) => s + p.publisher_earnings, 0);
            const fees = (payments||[]).reduce((s,p) => s + p.platform_fee, 0);
            const { count: docCount } = await supabase.from('pub_documents').select('id', { count: 'exact' }).eq('publisher_id', user.id);
            return res.status(200).json({ total_sales: total, your_earnings: earnings, platform_fees: fees, total_documents: docCount||0, total_transactions: (payments||[]).length, recent: (payments||[]).slice(0, 10) });
        }

        // ═══════════════════════════════════════════
        // PUBLISHER: Public page data
        // ═══════════════════════════════════════════
        ...
        if (path === '/api/pub/init-payment' && method === 'POST') {
            ...
            const finalAmount = Math.max(100, parseInt(doc.price));
            const pubEarnings = Math.floor(finalAmount * 0.93);
            const platFee = finalAmount - pubEarnings;

            await supabase.from('pub_payments').insert({ document_id, publisher_id: doc.publisher_id, device_fingerprint, client_token, amount: finalAmount, publisher_earnings: pubEarnings, platform_fee: platFee, currency: doc.currency, status: 'pending', customer_name: (customer_name||'').trim(), customer_phone: phone });
        ...
        if (path === '/api/pub/download' && method === 'POST') {
            ...
            if (!doc) return res.status(404).json({ error: 'Not found' });
            const { data: signedData } = await supabase.storage.from(PUB_BUCKET).createSignedUrl(doc.file_path, 600);
            if (!signedData) return res.status(500).json({ error: 'Link failed' });
        ...
        if (path === '/api/admin/upload' && method === 'POST') {
            ...
            let ce = fb.length; for (let i = fb.length - eb.length; i >= 0; i--) { if (fb.slice(i, i + eb.length).equals(eb)) { ce = i - 2; break; } }
            const bf = fb.slice(cs, ce); if (!bf || bf.length === 0) return res.status(400).json({ error: 'Empty file' });
            const sn = Date.now() + '_' + fn.replace(/[^a-zA-Z0-9._-]/g, '_');
            const { error } = await supabase.storage.from('documents').upload(sn, bf, { contentType: ft, cacheControl: '3600', upsert: false });
            if (error) return res.status(500).json({ error: 'Upload failed' });
            return res.status(200).json({ file_path: sn, filename: fn, size: bf.length });
        }
