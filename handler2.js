...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (path === '/api/rwandapay-callback' && method === 'POST') {
            ...
            if ((event === 'payment.successful' || event === 'payment.success') && data?.reference) { await supabase.from('payments').update({ status: 'completed', payment_method: 'rwandapay' }).or(`client_token.eq.${data.reference},transaction_ref.eq.${data.reference}`).eq('status', 'pending'); }
